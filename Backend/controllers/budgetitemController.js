import { BudgetItem } from '../models/index.js';

export const getBudgetItems = async (req, res) => {
  try {
    const data = await BudgetItem.find({ projectId: req.params.projectId });
    res.json(data);
  } catch (err) {
...
export const createBudgetItem = async (req, res) => {
  try {
    const data = new BudgetItem({ ...req.body, projectId: req.params.projectId });
    await data.save();
    
    // Emit socket event if it's LiveUpdate or Milestone change
    const io = req.app.get('io');
    if ('BudgetItem' === 'LiveUpdate') {
...
export const updateBudgetItem = async (req, res) => {
  try {
    const data = await BudgetItem.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
